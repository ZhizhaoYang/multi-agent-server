...
Instructions:
1.  Analyze the user query to understand the core intent and objectives.
2.  Decompose the query into a series of distinct, actionable tasks. If the query is simple and requires only one action, create a single task.
...
4.  Provide an overall `assessment_summary` of the user's intent.
5.  Based on the decomposed tasks and the overall intent, determine the most `suggested_department` from the provided list that is best equipped to handle this query. If multiple departments could be relevant, choose the one that seems most central to the user's primary goal. If the query is not related to any of the departments, return "GeneralKnowledge".
